...
function getPipelineStage(o) {
    ...
    if (status.includes('CANCELED') || status.includes('CANCELLED')) {
        return 'canceled';
    } else if (status.includes('RTO') || status.includes('DENIED') || status.includes('REACHED BACK')) {
        return 'denied';
...
function renderDashboard() {
    ...
    const deniedList = state.monthFilteredOrders.filter(o => {
        const s = (o.logisticsStatus || '').toUpperCase().trim();
        return (s.includes('RTO') || s.includes('DENIED') || s.includes('REACHED BACK')) && !s.includes('CANCELED') && !s.includes('CANCELLED');
    });
