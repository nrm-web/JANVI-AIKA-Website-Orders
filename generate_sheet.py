...
def process_and_create_excel():
    ...
    # ----------------------------------------------------
    # SECTION 6.5: CREATE CATEGORY SUMMARY SHEET
    # ----------------------------------------------------
    cat_summary_rows = []
    total_rev_all = df_consolidated["Total Price"].sum()
    
    # List of all categories to display
    all_categories = ["TOPS", "CHUDIDHAR", "ANARKALI", "LEHENGA", "CO-ORD SET", "HALF SAREE LEHENGA", "LONG GOWN", "SHARARA", "DRESS", "KURTI", "SAREE", "SET", "SUIT", "OTHERS"]
    
    # Explode multi-category strings if necessary or match exact category
    for cat in all_categories:
        if cat == "OTHERS":
            mask = df_consolidated["Category"].str.upper().str.contains("OTHER|-|UNKNOWN", na=False)
        else:
            search_term = "TOP" if cat == "TOPS" else cat
            mask = df_consolidated["Category"].str.upper().str.contains(search_term, na=False)
            
        cat_df = df_consolidated[mask]
        c_orders = len(cat_df)
        c_rev = cat_df["Total Price"].sum() if c_orders > 0 else 0.0
        c_deliv = len(cat_df[cat_df["Fulfillment Status"].str.upper().str.contains("DELIVERED|FULFILLED|SELF FULFILED", na=False)])
        c_share = (c_rev / total_rev_all) if total_rev_all > 0 else 0.0
        
        cat_summary_rows.append({
            "Category": cat,
            "Total Orders": c_orders,
            "Total Revenue": c_rev,
            "Delivered Orders": c_deliv,
            "Revenue Share (%)": c_share
        })
        
    df_cat_summary = pd.DataFrame(cat_summary_rows)
    df_cat_summary.to_excel(writer, sheet_name="Category Summary", index=False)
    sheet_cat = writer.sheets["Category Summary"]
    style_table(sheet_cat, df_cat_summary)
    
    # Custom Number Formatting for Category Summary
    for r_i in range(2, len(df_cat_summary) + 2):
        sheet_cat.cell(row=r_i, column=2).number_format = '#,##0'
        sheet_cat.cell(row=r_i, column=3).number_format = '₹#,##0.00'
        sheet_cat.cell(row=r_i, column=4).number_format = '#,##0'
        sheet_cat.cell(row=r_i, column=5).number_format = '0.0%'
        
    # Add Summary Row to Category Summary
    last_cat_row = len(df_cat_summary) + 1
    sum_cat_row = last_cat_row + 1
    sheet_cat.row_dimensions[sum_cat_row].height = 22
    sheet_cat.cell(row=sum_cat_row, column=1, value="Total").alignment = align_center
    c_tot_ord = sheet_cat.cell(row=sum_cat_row, column=2, value=f"=SUM(B2:B{last_cat_row})")
    c_tot_ord.alignment = align_center
    c_tot_ord.number_format = '#,##0'
    
    c_tot_rev = sheet_cat.cell(row=sum_cat_row, column=3, value=f"=SUM(C2:C{last_cat_row})")
    c_tot_rev.alignment = align_right
    c_tot_rev.number_format = '₹#,##0.00'
    
    c_tot_del = sheet_cat.cell(row=sum_cat_row, column=4, value=f"=SUM(D2:D{last_cat_row})")
    c_tot_del.alignment = align_center
    c_tot_del.number_format = '#,##0'
    
    c_tot_shr = sheet_cat.cell(row=sum_cat_row, column=5, value=f"=SUM(E2:E{last_cat_row})")
    c_tot_shr.alignment = align_center
    c_tot_shr.number_format = '0.0%'
    
    for col_idx in range(1, 6):
        cell = sheet_cat.cell(row=sum_cat_row, column=col_idx)
        cell.border = border_summary
        cell.font = font_bold
